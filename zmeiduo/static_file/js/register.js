var vm = new Vue({
    ...
    mounted: function () { //mounted方法是指加载完成后执行
        // 向服务器获取图片验证码
    ...
    methods: {
                ...
                axios.get(url, {
                    ...
                    .then(response => {
                        if (response.data.count > 0) {
                            ...
                            this.error_name = false;  //这句好像有点多余
                        }
                    })
                    .catch(error => { // status(非自定义)状态码是4xx,5xx都跑 .catch()
        ...
        check_phone: function () {
            console.log("进入check_phone this.error_phone=" + this.error_phone);
            var re = /^1[3-9]\d{9}$/;

            if (re.test(this.mobile)) {
                // this.error_phone = false;
                console.log("输入框匹配到手机号 准备请求检查数据库 this.error_phone=" + this.error_phone);
                var url = this.host + '/mobiles/' + this.mobile + '/count/';
                axios.get(url, {
                    responseType: 'json'
                })
                    .then(response => {
                        console.log("拿到数据库，准备解析 this.error_phone=" + this.error_phone);
                        if (response.data.count > 0) {
                            this.error_phone_message = '手机号已存在';
                            this.error_phone = true;
                            console.log("手机号在数据库 this.error_phone=" + this.error_phone);
                        } else {
                            this.error_phone = false;
                            console.log("手机号不在数据库 this.error_phone=" + this.error_phone);
                        }
                    })
                    .catch(error => {
                        console.log(error.response);
                ...
                this.error_phone = true;
                console.log("输入框匹配不到手机号 this.error_phone=" + this.error_phone);
            }
        ...
        send_sms_code: function () {
            ...
            // 校验参数，保证输入框有数据填写
            var ttest = this.check_phone();
            this.check_image_code();
            console.log("短信检验位置 this.error_phone=" + this.error_phone + "check_phone返回值=" + ttest)
            if (this.error_phone == true || this.error_image_code == true) {
                console.log("因为有true所以阻止请求短信" + this.error_phone + this.error_image_code);
            ...
            axios.get(url, {
                ...
                .then(response => {
                    ...
                    if (response.data.code == '0') {
                        ...
                        //arguments:60 没有定义参数可以调用？感觉可以去掉前面
                    } else {

                        console.log('获取短信 接口报错');
                        if (response.data.code == '4002') {
                            this.error_image_code_message = response.data.errmsg;
                            this.error_image_code = true;
                            // parseInt str转int，不过字典过来的是数字可以直接用
                            // var num = parseInt(response.data.sms_code_delay);
                            var num = response.data.sms_code_delay;
                            var t = setInterval(() => {
                                if (num == 1) {
                                    clearInterval(t);
                                    // 将点击获取验证码的按钮展示的文本回复成原始文本
                ...
                .catch(error => {
                    console.log('获取短信 处理逻辑出错' + error.response);
                    this.sending_flag = false;
                })
        },
        // 表单提交
